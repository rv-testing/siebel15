...
import time, sys, re, os, fnmatch, csv, shutil
...
#######
debug=0  # turn off  dubug messages
#debug=1 # turn on  dubug messages
...
output_log='/tmp/srchPtrn-output1.log'   #will generate temp output file 
...
file_last_rd_pos=os.path.dirname(sys.argv[0])+'/srchPtrn.confg'  ##keep the last position file in the script dir
...
if os.path.isfile(file_last_rd_pos):    #last poistion file exist or not
	...
	with open(file_last_rd_pos) as csvfile:
...
for index, elem in enumerate(last_read_pos_filename_a):
	if debug > 0:
		print 'last_read_pos_num_a[index]: ',str(last_read_pos_num_a[index])
